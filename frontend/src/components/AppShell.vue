<template>
  <v-app :theme="currentTheme">
    ...
    <v-app-bar flat border="b" height="56" class="bg-surface">
      <v-app-bar-title>
        <span class="bookgram-logo">Bookgram</span>
      </v-app-bar-title>
      <template #append>
    ...
    >
      ...
      <v-btn value="activities" @click="$router.push('/activities')">
        <v-badge :content="pendingCount" :model-value="pendingCount > 0" color="warning" floating>
          <v-icon size="26">mdi-clipboard-list-outline</v-icon>
        </v-badge>
      </v-btn>
...
async function loadNotifications() {
  ...
  try {
    ...
    pendingCount.value = acts.filter((a: any) => a.status === 'PENDING').length
  } catch {}
}
